...
use crate::snapshot::{NodeSnapshot, RectSnapshot, SceneSnapshot};
...
"#;
    ...
    #[test]
    fn structure_map_shows_button() {
        let s = HeadlessSession::new(BUTTON_BUNDLE, 400, 300);
        let snap = s.snapshot();
        let map = structure(&snap, 40, 10);
        assert!(
            map.contains("[Click]"),
            "structure map should show [Click]:\n{}",
            map
        );
    }

    #[test]
    fn structure_map_correct_dimensions() {
        let s = HeadlessSession::new(BUTTON_BUNDLE, 400, 300);
        let snap = s.snapshot();
        let map = structure(&snap, 40, 10);
        assert_eq!(map.lines().count(), 10);
        for line in map.lines() {
            assert_eq!(line.len(), 40, "line len mismatch: {:?}", line);
        }
    }

    #[test]
    fn render_map_has_content_for_button() {
        let mut s = HeadlessSession::new(BUTTON_BUNDLE, 400, 300);
        if let Some(pixels) = s.render_pixels() {
            let map = render(&pixels, 400, 300, 40, 10);
            let non_space = map.chars().filter(|c| *c != ' ' && *c != '\n').count();
            assert!(
                non_space > 0,
                "render map should have non-space chars for button:\n{}",
                map
            );
        }
    }
    ...
    #[test]
    fn overlay_map_combines_pixels_and_labels() {
        let mut s = HeadlessSession::new(BUTTON_BUNDLE, 400, 300);
        let snap = s.snapshot();
        if let Some(pixels) = s.render_pixels() {
            let map = overlay(&snap, &pixels, 400, 300, 40, 10);
            // Should have both density chars from pixels AND label text
            let has_bracket = map.contains('[');
            let has_density = map.chars().any(|c| "·░▒▓█".contains(c));
            assert!(
                has_bracket || has_density,
                "overlay should combine pixels and labels:\n{}",
                map
            );
        }
    }
    ...
    #[test]
    fn validate_map_empty_scene_no_issues() {
        let mut s = HeadlessSession::new("function rvst_mount(t) {}", 400, 300);
        let snap = s.snapshot();
        if let Some(pixels) = s.render_pixels() {
            let (_, issues) = validate(&snap, &pixels, 400, 300, 40, 10);
            assert!(
                issues.is_empty(),
                "empty scene should have no validation issues"
            );
        }
    }
}
