...
// ── Blitz DOM adapter ──────────────────────────────────────────────────
//
// When the `blitz` feature is enabled, SceneSnapshot can be built from
// a Blitz BaseDocument instead of an rvst-tree Tree. This enables the
// entire toolchain (ascii, inspector, test harness, analysis) to work
// with the new Blitz rendering backend.

#[cfg(feature = "blitz")]
mod blitz_adapter {
    use super::*;
    use blitz_dom::{BaseDocument, NodeData, local_name};
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};

    impl SceneSnapshot {
        /// Build a snapshot from a Blitz BaseDocument.
        pub fn from_blitz_doc(doc: &BaseDocument, viewport_w: f32, viewport_h: f32) -> Self {
            let focused_id = doc.get_focussed_node_id().map(|id| id as u32);
            let tree = doc.tree();
            let mut nodes = Vec::new();
            let mut root_children = Vec::new();

            // Find <body> element — our mount root
            let root_element = doc.root_element();
            for &child_id in &root_element.children {
                if let Some(node) = tree.get(child_id) {
                    if let NodeData::Element(data) = &node.data {
                        if *data.name.local == *"body" {
                            root_children = node.children.iter().map(|&id| id as u32).collect();
                        }
                    }
                }
            }

            // Walk all nodes in the slab
            for (node_id, node) in tree.iter() {
                ...
                let (node_type, text, tag) = match &node.data {
                    NodeData::Document => continue,
                    NodeData::Comment => continue,
                    NodeData::AnonymousBlock(_) => continue,
                    NodeData::Text(data) => {
                    ...
                    NodeData::Element(data) => {
                        ...
                        let node_type = match tag.as_str() {
                            ...
                            "form" => "Form",
                            "div" | "span" | "main" | "aside" | "section" | "nav"
                            | "header" | "footer" | "article" | "p" | "h1" | "h2"
                            | "h3" | "h4" | "h5" | "h6" | "ul" | "ol" | "li"
                            | "a" | "label" | "html" | "head" | "body" | "style" => "View",
                            _ => "View",
                        }
                        .to_string();
                        (node_type, None, Some(tag))
                    }
                };

                // Extract styles from computed values
                let mut styles = HashMap::new();
                if let Some(s) = node.primary_styles() {
                    let display = s.clone_display();
                    styles.insert("display".into(), format!("{display:?}"));
                    if let Some(tag_name) = &tag {
                        styles.insert("tag".into(), tag_name.clone());
                    }
                    // Extract class attribute
                    if let NodeData::Element(data) = &node.data {
                        if let Some(class) = data.attr(local_name!("class")) {
                            if !class.is_empty() {
                                styles.insert("class".into(), class.to_string());
                            }
                        }
                        if let Some(role) = data.attr(local_name!("role")) {
                            styles.insert("role".into(), role.to_string());
                        }
                    }
                }
                ...
                // Compute accessible name
                let name = if let Some(ref t) = text {
                    Some(t.clone())
                } else {
                    // Collect child text
                    let child_text: String = node
                        ...
                        .filter_map(|c| {
                            if let NodeData::Text(data) = &c.data {
                                let t = data.content.trim();
                                if !t.is_empty() {
                                    Some(t.to_string())
                                } else {
                                    None
                ...
                nodes.push(NodeSnapshot {
                    ...
                    node_type,
                    text,
                    parent: node.parent.map(|p| p as u32),
                    children: node.children.iter().map(|&c| c as u32).collect(),
                    styles,
                    layout: if has_layout {
                        Some(RectSnapshot { x, y, w, h })
                    } else {
                        None
                    },
                    scroll_y: 0.0,
                    focused: is_focused,
                    has_handlers: false, // TODO: wire from bridge listeners
                    semantic_id,
                    role,
                    name,
                });
