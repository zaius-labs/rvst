...
impl DocumentMutator<'_> {
    ...
    pub fn create_element(&mut self, name: QualName, attrs: Vec<Attribute>) -> usize {
        ...
        // Clone default styles before mutably borrowing the document for node creation
        let default_styles = self.doc.stylist.device().default_computed_values_arc().clone();

        let id = self.doc.create_node(NodeData::Element(data));
        let node = self.doc.get_node_mut(id).unwrap();

        // Initialise style data with default primary styles so Stylo's
        // invalidation traversal doesn't panic when checking is_display_none()
        // on newly-created elements that haven't been styled yet.
        let mut element_data = style::data::ElementData::default();
        element_data.damage = ALL_DAMAGE;
        element_data.styles.primary = Some(default_styles);
        *node.stylo_element_data.ensure_init_mut() = element_data;
